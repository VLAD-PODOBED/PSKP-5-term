...
server.register('sum', (params) => {
    let sum = 0;
    params.forEach(item =>{ if(Number.isInteger(item)) sum += item;});
    return sum;
}).public();

server.register('mul', (params) => {
    let mul = 1;
    params.forEach(item => {if(Number.isInteger(item)) mul *= item;});
    return mul;
}).public();

server.register('fib', (param) => {
    if (param.length === 1 ) return fib(param)
    else return [1];
}).protected();

server.register('fact', (param) => {
    if (param.length === 1) return factorial(param)
    else return [1];
}).protected();